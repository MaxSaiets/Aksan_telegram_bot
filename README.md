...
- each new YouTube description is generated by Gemini when `GEMINI_API_KEY` is configured; an offline varied fallback keeps uploads working during an API outage
...
For AI descriptions, add the `GEMINI_API_KEY` repository secret in GitHub. The
deploy workflow writes it to the ignored server `.env` before restarting the
bot services; it is never committed to the repository.
