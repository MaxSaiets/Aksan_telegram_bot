...
def test_process_photo_resizes_and_outputs_jpg(tmp_path):
    ...
    out = process_photo(src, "25.3251_norma_ifsh", 1)
...
def test_photo_pipeline_runs_with_mocked_dependencies(tmp_path, monkeypatch):
    ...
    def mock_save_batch(**kwargs):
        saved_batch_calls.append(kwargs)
        return {"id": "batch-1234", "model_code": "25.3251"}
    ...
    monkeypatch.setattr("app.tasks.photo_pipeline.photo_library_repo.save_batch", mock_save_batch)
    ...
    assert result["batch_id"] == "batch-1234"
    assert len(sent_batches) == 1
    assert sent_batches[0][1] == "25.3251_norma_ifsh"
    assert len(sent_batches[0][0]) == 2
    assert any("[1/4]" in message for message in sent_messages)
    assert len(saved_batch_calls) == 1
    assert saved_batch_calls[0]["source_file_ids"] == ["photo1", "photo2"]
    assert saved_batch_calls[0]["target_message_ids"] == [101, 102]
    assert saved_batch_calls[0]["caption_message_id"] == 103
